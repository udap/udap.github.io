...
第一个问题正在通过一些解决方案来解决，例如基于POS的共识和一些根本不使用区块链的协议。 UDAP特别使用[Cosmos](http://cosmos.network/)作为通用网络体系结构。 Cosmos提供了UDAP可以立即利用的以下功能：
1.提供1000+ TPS性能的POS共识实施。
2.提供_finality_的API，这是UDAP成为易于使用的API的必备条件。
3. Cosmos可以通过向区块链中心添加更多“区域”来线性缩放。

对于第二个因素，缺乏事务分界的根本原因是因为在一般区块链中没有足够的关于账户依赖关系的信息。一笔交易可能将资金存入一个账户，而另一笔交易则将资金从账户中取出。在同一个帐户上工作需要同步，这样基本上序列化了对帐户的访问。
...
![](media/UDAP parallel chains.png)

在UDAP中，每个应用程序都分配了一个应用程序ID并拥有一个独立的链，这意味着如果有一千个应用程序注册，UDAP网络中将会有一千个区块链。每个链都有自己的状态树分支，存储树和交易收据。每个应用程序中的状态转换发生在一个单独的线程中。
...
上述问题可以通过UDAP节点中的智能线程分派器来解决，假定UDAP API一次主要影响一个账户。调度程序仔细检查传入的交易，并通过受影响的账户进行分区。只要事务分区仍然可行，事务验证和分块结构仍然可以并行完成。

由于在所有节点之间没有竞争性的重复计算，并且在节点软件中的事务处理中具有优化的并行性，所以UDAP在水平和垂直方向都提供了显着的更好的可伸缩性。我们预计UDAP网络将在全球600个节点的网络上提供10K〜100K范围内的TPS。

与此同时，我们正密切关注由Joseph Poon和Vitalik Buterin领导的[Plasma Project](http://plasma.io/)的进展。等离子项目提出了基于区块链的递归Map-Reduce体系结构，旨在提供高达数十亿级别的TPS。等离子的发展将成为2018年行业中最优秀人才团队的核心努力之一。我们计划利用未来的工作来解决可扩展性问题。
